...
module.exports = function redirectify( b,opts ) {
    ...
    let moduleRoots = (()=>{
        ...
        if(!Array.isArray(options.moduleroot) || options.moduleroot.length<=0)return;
        let result = options.moduleroot.filter(root=>typeof root==="string" || root==="node_modules")
        if( result.length<=0 )return
        return result.map(root=>root.replace(/\*$/,"")).concat(["node_modules"]);
    })()
    ...
    b._createDeps = function(opts){
        ...
        mdeps.resolver = function (id, parent, cb) {
            ...
            parent.moduleDirectory = moduleRoots;
