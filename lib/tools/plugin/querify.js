...
function browserifyPlugin(b, opts){
    opts = xtend(opts);
    let optionsResolver = (typeof opts.replacer==="function")?opts.replacer:noopReplacer;
    let replaceHandeler = replaceHandelerCreater("",xtend(opts,{replacer:optionsResolver}));
    let redirectedFiles = {};
    ...
    b.redirectReadingFile(id=>praseQuerySplit(id).query!==void 0
        ,function(file,id){
            ...
            replaceHandeler("(\""+file+"\");",file,function(err,result){
    ...
    b.transform(function (file, opts) {
        ...
        return transform(file, xtend(opts, {
            replacer: function replaceFunctionArguments(fullPath, destPath, keyword, queryOptions) {
...
/**
 * 添加一只内容替换机
 * @param checker:filepath=>boolean
 * @param replacer:(file,contents,opts,done)=>void
 */
function addContentReplacer(checker,replacer){
    let self = this||addContentReplacer;
    if(!Array.isArray(self._replacers))self._replacers=[];
    self._replacers.push(Promise.promisify(function(file,contents,opts,done){
        if(checker(file)){
            return replacer(file,contents,opts,done)
        }
        return done(null,contents)
    }));

    self.replacers = function(file,contents,opts){
        let lastContents = contents;
        return new Promise(function(resolve){
            Promise.reduce(self._replacers,function(curr,next){
...
addContentReplacer(filepath=>minimatch(filepath, "*.css", { matchBase: true })
    , function (filepath, contents, opts, done) {
        let replaceHandeler = replaceHandelerCreater(filepath, xtend(opts,{keyword:"url"}))
...
addContentReplacer(filepath=>minimatch(filepath, "*.js", { matchBase: true })
    , function (filepath, contents, opts, done) {
        if(opts)opts.keyword = "";
        let replaceHandeler = replaceHandelerCreater(filepath, opts)
...
function transform(file, opts){
    let replacers = opts.replacers||addContentReplacer.replacers;
 ...
 */
function replaceHandelerCreater(file, opts){
    opts = xtend(opts)
    let replacer = (typeof opts.replacer==="function")?opts.replacer:noopReplacer;
    ...
    return function replaceHandeler(propDefault,propPath,done){
        ...
        if(!isUrlAbsolute(fullPath)){
            ...
            let resolveResult = replacer(fullPath,destPath,keyword,queryOptions);
