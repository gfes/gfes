...
module.exports = function (b, opts) {
    ...
    b.while("*.json",function(s,file,id,params){
        if(params["key"]){
            ...
            return s.transform(through(function(buf, env, next){
...
class Queryifyer {
    _redirectFile(file,id,options){
        if (fs.existsSync(file) && options._params) {
            ...
            frs.transform = function(tr,opts){
                let fn = tr;
                if(isReadableStream(tr)){
                    fn=()=>tr;
                }
                return frs.pipe(fn(file,opts));
            }
